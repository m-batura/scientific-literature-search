...
path_to_json = '.\\saves\\titles.json'
path_to_parquet = '.\\saves\\titles.parquet'
...
def first_retrieval(df, embedding):
    df['title_dist'] = df['title_embed'].apply(lambda x: met.dot(x, embedding))
...
def second_retrieval(df, embedding):
    ...
    df['abstract_dist'] = df['abstract_embed'].apply(lambda x: met.dot(x, embedding))
...
if __name__ == "__main__":
    ...
    df_result = df_second[['title', 'title_dist', 'abstract', 'abstract_dist']]
    df_result['abstract'] = df_result['abstract'].str[:300]
    # pd.set_option('display.max_colwidth', 20)
    # print(df_second)
    df_result.to_csv('.\\results\\sim_search_short.csv', index=False, float_format="%.4f")
    0